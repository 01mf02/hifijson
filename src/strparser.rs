...
pub trait LexerStr: Lexer {
    ...
    fn str_string(&mut self) -> Result<Self::Str, error::Str>;
}
...
impl<'a> LexerStr for SliceLexer<'a> {
    ...
    fn str_string(&mut self) -> Result<Self::Str, error::Str> {
        ...
        self.str_fold(Cow::Borrowed(""), on_string, |lexer, escape, out| {
            out.to_mut()
                .push(lexer.escape_char(escape).map_err(error::Str::Escape)?);
            Ok(())
        })
    }
}
...
impl<E, I: Iterator<Item = Result<u8, E>>> LexerStr for IterLexer<E, I> {
    ...
    fn str_string(&mut self) -> Result<Self::Str, error::Str> {
        ...
        self.str_fold(Self::Str::new(), on_string, |lexer, escape, out| {
            out.push(lexer.escape_char(escape).map_err(error::Str::Escape)?);
            Ok(())
        })
    }
}
